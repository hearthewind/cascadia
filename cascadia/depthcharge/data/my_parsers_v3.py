    ...
    def read(self) -> BaseParser:
        ...
        for index, row in tqdm(spec_header.iterrows(), total=len(spec_header), desc=f'Reading file {self.path.name}'):
            ...
            for _, spectrum in related_ms1.iterrows():
                ...
                chosen_mzs = spectrum.mz[sorted_intensity_idxs]
                chosen_intensities = spectrum.intensity[sorted_intensity_idxs]

                sorted_mz_idxs = np.argsort(chosen_mzs)
                chosen_mzs = chosen_mzs[sorted_mz_idxs]
                chosen_intensities = chosen_intensities[sorted_mz_idxs]
                ...
                ms1_mzs.append(list(chosen_mzs))
                ms1_intensities.append(list(chosen_intensities))
                ms1_rts.append([scan_rt - precursor_rt] * len(sorted_intensity_idxs))
                ms1_rts_single.append(scan_rt - precursor_rt)
                ms1_levels.append([1] * len(sorted_intensity_idxs))
                ms1_fragment_labels.append([0] * len(sorted_intensity_idxs))

            abs_rts = [np.abs(x) for x in ms1_rts_single]
            sorted_rt_idxs = np.argsort(abs_rts)[:(scan_width + 1)]
            ...
            for _, spectrum in related_ms2.iterrows():
                ...
                ms2_mzs.append(list(chosen_mzs))
                ms2_intensities.append(list(chosen_intensities))
                ms2_rts.append([scan_rt - precursor_rt] * len(sorted_intensity_idxs))
                ms2_rts_single.append(scan_rt - precursor_rt)
                ms2_levels.append([2] * len(sorted_intensity_idxs))
                ms2_fragment_labels.append([0] * len(sorted_intensity_idxs))
